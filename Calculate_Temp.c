...
void Calculate_Temp(int *tlist, double *vlist,int nfac,int nvert,double* angles,double* E0,double t0,double Gamma, double A,double R,int N,double *Tres)
{
 ...
 //FindActualBlockers(tlist,vlist,nfac,nvert,E0,E0,N,visible);
  ...
  fft_out=(kiss_fft_cpx*)malloc(sizeof(kiss_fft_cpx)*N);
  ...
  for(int facet=0;facet<nfac;facet++)
  {
    ...
    kiss_fftr(cfg,&(mu0[facet*N]),fft_out);
    for(int j=0;j<N/2;j++)
    dn[j]=fft_out[j].r+I*fft_out[j].i;
...
void Calculate_Temp_deriv(int *tlist, double *vlist,int nfac,int nvert,double* angles,double* E0,double t0,double Gamma, double A,double R,int N,double *Tres,double *Tresdx,double *Tresdy,double *Tresdz, double *TresdA)
{
 ...
 for(int facet=0;facet<nfac;facet++)
 {
     ...
     kiss_fftr(cfg,&(dmudx1[facet*N]),fft_out);
     for(int j=0;j<N/2;j++)
      dndx1[j]=fft_out[j].r+I*fft_out[j].i;
     
     
     kiss_fftr(cfg,&(dmudx2[facet*N]),fft_out);
     for(int j=0;j<N/2;j++)
      dndx2[j]=fft_out[j].r+I*fft_out[j].i;
     
      kiss_fftr(cfg,&(dmudx3[facet*N]),fft_out);
     for(int j=0;j<N/2;j++)
      dndx3[j]=fft_out[j].r+I*fft_out[j].i;
     
      kiss_fftr(cfg,&(dmudy1[facet*N]),fft_out);
     for(int j=0;j<N/2;j++)
      dndy1[j]=fft_out[j].r+I*fft_out[j].i;
     kiss_fftr(cfg,&(dmudy2[facet*N]),fft_out);
     for(int j=0;j<N/2;j++)
      dndy2[j]=fft_out[j].r+I*fft_out[j].i;
     kiss_fftr(cfg,&(dmudy3[facet*N]),fft_out);
     for(int j=0;j<N/2;j++)
      dndy3[j]=fft_out[j].r+I*fft_out[j].i;
      kiss_fftr(cfg,&(dmudz1[facet*N]),fft_out);
     for(int j=0;j<N/2;j++)
      dndz1[j]=fft_out[j].r+I*fft_out[j].i;
     kiss_fftr(cfg,&(dmudz2[facet*N]),fft_out);
     for(int j=0;j<N/2;j++)
      dndz2[j]=fft_out[j].r+I*fft_out[j].i;
     kiss_fftr(cfg,&(dmudz3[facet*N]),fft_out);
     for(int j=0;j<N/2;j++)
      dndz3[j]=fft_out[j].r+I*fft_out[j].i;
      kiss_fftr(cfg,&(dmudb[facet*N]),fft_out);
     for(int j=0;j<N/2;j++)
      dndb[j]=fft_out[j].r+I*fft_out[j].i;
     kiss_fftr(cfg,&(dmudl[facet*N]),fft_out);
     for(int j=0;j<N/2;j++)
      dndl[j]=fft_out[j].r+I*fft_out[j].i;
