...
int main()
{
   ...
   // printf("last datai: %f\n",AO->datai[0][AO->nobs[0]-1]);
    ...
    for(int k=0;k<NumofRounds;k++)
    {
        ...
        Sqrt3_Subdiv(tlist,vlist,nfac,nvert,&tlistn,&vlistn,&nfacn,&nvertn,&D,2); //Do 2x subdivision. Remember to free allocated mem
        //Calculate LCs
...
//         write_matrix_file("/tmp/X.txt",X,1,nJcols);
        ...
        Sqrt3_Subdiv(tlist,vlist2,nfac,nvert,&tlistn,&vlistn,&nfacn,&nvertn,&D,2);
