 ...
 if(A!=NULL)
 {
  ...
  int vr1,vr2,vr3;
	...
	/*Brightness*/
        ...
        {
            alb=A[j];
            dSdalb[e*numfac+j]=(ha-la)*exp(alb)/pow(exp(alb)+1,2)*mu*mu0*(1/(mu+mu0)+c)*area[j];
        }
	In=(la+(ha-la)*exp(alb)/(exp(alb)+1))*mu*mu0*(1/(mu+mu0)+c);
	tb[e]=tb[e]+In*area[j];
	
	dt1=(la+(ha-la)*exp(alb)/(exp(alb)+1))*(pow(mu/(mu+mu0),2)+c*mu);
	dt2=(la+(ha-la)*exp(alb)/(exp(alb)+1))*(pow(mu0/(mu+mu0),2)+c*mu0);
